...
export interface EditBatchReminderInput {
  id: string;
  title: string;
  subject: string;
  content: string;
...
/**
 * Edits a reminder scoped to a batch assignation. Ensures the batch has its
 * own cloned reminder set first so the edit never touches the global reminders.
 */
export async function editBatchReminderAction(
  batchId: string,
  input: EditBatchReminderInput,
  ...
  try {
    ...
    // Ensure batch-specific reminders exist before editing.
    ...
    if (!existingReminder) {
      return {
        ...
        message: {
          title: "Not Found",
          description: "Reminder not found for this batch.",
        },
      };
    }
    ...
    return {
      ...
      message: {
        title: "Success",
        description: "Reminder updated for this batch.",
      },
    };
  } catch (error) {
    console.error("Error editing batch reminder:", error);
    return {
      status: "error",
      message: {
        title: "Error",
